...
def _tga_basename_validation(cluster, dependency_usage):
    expected = Path(cluster).stem.casefold()
    ...
    invalid = [
        ...
        or not (
            path.stem.casefold() == expected
            or path.stem.casefold().startswith(expected + "_")
        )
    ]
    if missing:
        status = "missing"
    ...
    return {
        "status": status,
        "expected_base": Path(cluster).stem,
...
def build_cluster_assembly_contract(
    ...
    clusters = [Path(path) for path in clusters or []]
    usage = cluster_usage
    if usage is None:
    ...
    relevant_clusters = [
        ...
        if str(cluster).casefold() in usage
        and dependency_role(cluster.stem) in ROLE_ORDER
    ]
    export_bundles = {
    ...
    for cluster in relevant_clusters:
        key = str(cluster).casefold()
        role = dependency_role(cluster.stem)
        dependency_usage = usage[key]
        expected_identity = _expected_role_identity(folder, role)
        primary_role_source = (
            normalize_export_name(cluster.stem)
    ...
    if "blocked" in decisions or bark["status"].startswith("blocked"):
...
def locate_cluster_assembly_receipt(spm_path, receipt_dir=None):
    """Locate exactly one current receipt for an SK_ or authoritative SPM."""
    ...
    if len(matches) != 1:
        raise ClusterAssemblyReceiptError(
            f"multiple Cluster Assembly receipts identify SPM: {spm_path}")
