...
class StateSaveBatchingTests(unittest.TestCase):
    ...
    def test_routine_assembly_updates_save_once_at_phase_boundary(self):
        gui = load_gui_module()
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            def fake_job(iid, _spm, _item):
                with app.state_lock:
                    app.state.setdefault(iid, {})["blend_status"] = "routine update"
                    app._save_state_after_phase_update()

            app._job_blender = mock.Mock(side_effect=fake_job)
            with mock.patch.object(
                gui, "LOG_DIR", root / "logs"
            ), mock.patch.object(
                gui,
                "save_state",
                side_effect=lambda state: saved_states.append(copy.deepcopy(state)),
            ):
                result = app._run_batch("blender", targets, emit_done=False)

        self.assertTrue(result)
        self.assertEqual(len(saved_states), 1)
        self.assertTrue(all(
            saved_states[0][str(item["spm"])]["blend_status"] == "routine update"
            for item in targets
        ))
    ...
    def test_failure_is_saved_immediately_while_routine_updates_are_batched(self):
        ...
        self.assertEqual(
            saved_states[-1][str(failed)]["blend_status_error"]["kind"],
            "data_error",
        )
    ...
    def test_cancelled_unstarted_rows_are_saved_at_phase_exit(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.object(
                ...
                result = app._run_batch("blender", [target], emit_done=False)

        self.assertTrue(result)
        self.assertEqual(len(saved_states), 1)
        record = saved_states[0][str(spm)]["blend_status_result"]
        self.assertEqual(record["kind"], "cancelled")
