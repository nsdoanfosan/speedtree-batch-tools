...
넣기 전에 제외한다. 이 빠른 판정은 SPM/Blend 내용을 다시 읽거나 해시하지 않고
파일 크기·수정시간·파일 ID와 스캔에서 이미 확보한 Cluster 대상 인덱스만 비교한다.
의존 SPM과 Cluster 관계 신호가 달라진 행은 실패시키지 않고 등록 관계를 자동
갱신한 다음 현재 출력 계약을 다시 판정한다. 관계만 달라졌고 기존 Blend가 여전히
유효하면 그대로 스킵하며, SPM·Blend·보고서·출력 설정처럼 BWR 산출물에 직접
영향을 주는 입력이 달라진 경우에만 Repair를 다시 실행한다. 영수증은 빠른 스킵
힌트일 뿐 실패 근거가 아니다. 기존 상태 파일은 관계 비대상 행의 첫 라이브 상태
감사 또는 관계 대상 행의 첫 실제 관계 검증에서 영수증을 자동 생성한다.
