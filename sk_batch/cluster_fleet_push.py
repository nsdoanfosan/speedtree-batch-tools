...
def validate_transport_batch_scope(transport, targets, provider_spms):
    """Keep live-editor RPC available for one exact asset, never a fleet."""
    work_count = len(list(targets or [])) + len(list(provider_spms or []))
    if transport == "rpc" and work_count > 1:
        raise ExactPushError(
            "fleet RPC import is disabled because it accumulates skeletal, "
            "Nanite, ray-tracing, and viewport resources in one editor; use "
            "the default headless -NullRHI transport, or select one exact "
            "asset with no provider dependency"
        )
    return {"transport": transport, "work_count": work_count}
...
def main(argv=None):
    ...
    validate_transport_batch_scope(args.transport, targets, provider_spms)
