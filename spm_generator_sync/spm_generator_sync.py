...
def _atlas_target_relation_records(spm_path: Path) -> list[dict]:
    """Load non-conflicting exact authority for one Atlas target."""
...
def _atlas_target_relation_manifest(spm_path: Path) -> dict:
    """Load non-blocking Atlas protection evidence for one exact target."""
    resolution = resolve_atlas_manifests(
        spm_path,
        require_generator_complete=True,
        diagnostic_only=True,
    ...
    for record in selected:
        protection_bindings.extend(
            manifest_current_generator_bindings(record.get("payload") or {})
        )
...
class SPMDocument:
    def __init__(
        ...
        ownership_mutation_authorized = self.atlas_relation_manifest.get(
            "_atlas_relation_mutation_authorized", True
        )
        for record in self.atlas_relation_records:
            if not ownership_mutation_authorized:
                continue
            ...
            for binding in current_bindings:
                ...
                if (
                    previous is None
                    or "_atlas_source_identity_key" not in previous
                ):
...
def strip_atlas_created_variant_slots_from_clone(
    ...
    bindings.extend(
        ...
        and (binding_guid, str(binding.get("slot_prefix") or "").strip())
        not in created_keys
    )
