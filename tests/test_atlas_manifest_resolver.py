...
class AtlasManifestResolverTests(unittest.TestCase):
    ...
    def test_diagnostic_projection_filters_explicit_current_ownership_only(self):
        with tempfile.TemporaryDirectory() as temporary:
            root, target, _blend, payload = self.fixture(temporary)
            authority_binding = copy.deepcopy(
                payload["generator_connection"]["bindings"][0]
            )
            payload["generator_binding_ownership"] = self.ownership_block(
                [authority_binding]
            ...
            mixed = copy.deepcopy(payload)
            conflicting = copy.deepcopy(authority_binding)
            conflicting["target_mesh_id"] = 99
            unique = {
                "generator_guid": "branch-guid",
                "slot_prefix": "Branches:Type:4",
                "target_material_id": 8,
                "target_mesh_id": 30,
            }
            mixed["material_groups"][0]["mesh_ids"] = [99]
            mixed["generator_connection"]["bindings"] = [
                conflicting,
                unique,
            ]
            mixed["generator_binding_ownership"] = self.ownership_block(
                [conflicting, unique]
            )
            provider = self.write_candidate(
                root,
                ...
                "exact_target_scope",
                mixed,
                name=f"mixed-explicit__{target.stem}.json",
            )
            ...
            projected = resolution["selected"][1]["payload"]
            self.assertEqual(
                projected["generator_binding_ownership"]["bindings"],
                [unique],
            )
            self.assertEqual(
                projected["generator_binding_ownership"]["binding_count"],
                1,
            )
            self.assertEqual(
                projected["generator_binding_ownership"]["fingerprint"],
                generator_binding_ownership_fingerprint([unique]),
            )
            self.assertEqual(
                projected["generator_connection"]["bindings"],
                [conflicting, unique],
                "authored/run evidence must remain immutable",
            )
